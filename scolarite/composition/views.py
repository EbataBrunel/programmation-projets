...
def detail_cmp_trimestre(request, salle_id, trimestre):
    ...
    students = (
        Composer.objects.values("student_id")
            .filter(anneeacademique_id=anneeacademique_id, salle_id=salle_id, trimestre=trimestre)
    ...
    for matiere in matieres:
        ...
        evaluations = (
            Composer.objects.values('evaluation')
                .filter(anneeacademique_id=anneeacademique_id, salle_id=salle_id, trimestre=trimestre, matiere_id=matiere["matiere_id"])
            ...
            if evaluation["evaluation"] == "Contrôle":
                nb_comp_numcontrole = (
                    Composer.objects.values('numerocontrole')
                        .filter(anneeacademique_id=anneeacademique_id, salle_id=salle_id, trimestre=trimestre, matiere_id=matiere["matiere_id"], evaluation=evaluation["evaluation"])
...
def detail_comp_student(request, salle_id, trimestre, student_id):
    ...
    compositions = Composer.objects.filter(anneeacademique_id=anneeacademique_id, salle_id=salle_id, student_id=student_id, trimestre=trimestre)
    anneeacademique = AnneeCademique.objects.get(id=anneeacademique_id)
    context = {
            "setting": setting,
            "compositions": compositions,
...
def detail_comp_matiere(request, salle_id, trimestre, matiere_id):
    ...
    compositions = Composer.objects.filter(
        ...
        salle_id=salle_id, matiere_id=matiere_id, 
        trimestre=trimestre)
...
def add_composition(request):
    ...
    if request.method == "POST":
        ...
        deliberation = Deliberation.objects.filter(salle_id=salle_id, trimestre=trimestre, status_cloture=True, anneeacademique_id=anneeacademique_id)        
        ...
        if evaluation == "Examen":   
            ...
            if query.exists():
                return JsonResponse({
                    "status": "error",
                    "message": "okCette composition existe déjà."})
    ...
    if request.session.get('group_name') in permission_enseignant:
...
def edit_composition(request, id, param):
    ...
    """trimestres = []
    if classe.libelle.capitalize() in ["Seconde", "Première"]:
        trimestres = ["Trimestre 1", "Trimestre 2", "Trimestre 3"]
    elif classe.libelle.capitalize() == "Terminale":
        trimestres = ["Trimestre 1", "Trimestre 2", "Bac Test", "Bac Blanc"]
    else:
        trimestres = ["Trimestre 1", "Trimestre 2", "Bac Test", "Bac Blanc"]"""
    ...
    trimestre_simplifier = []
    for trimestre in trimestres_enseignements:
        trimestre_simplifier.append(trimestre['trimestre'])
    ...
    if request.session.get('group_name') in permission_directeur_etudes:    
        ...
        for trimestre in trimestre_simplifier:
            if classe.libelle.capitalize() == "Terminale" and composition.trimestre == "Trimestre 3":
                trimestres.extend(["Bac Test", "Bac Blanc"])
            else:
                trimestres.append(trimestre)
    else:
...
def matiere_enseigner(anneeacademique_id, salle_id, trimestre):
    enseignements = Enseigner.objects.filter(salle_id=salle_id, anneeacademique_id=anneeacademique_id, trimestre=trimestre)
    matieres = []
...
# Calculer la somme finale des notes
def cacluler_somme_finale_notes(anneeacademique_id, salle_id, trimestre, student_id):
    somme_note_finale = float(0)
    # Récuperer les matières qui ont été enseignées et composer par les étudiants
    matiers = matiere_enseigner(anneeacademique_id, salle_id, trimestre)
    for matiere in matiers:
        somme_note_finale += float(calucler_note_finale_matiere(anneeacademique_id, salle_id, matiere.id, trimestre, student_id))
    return somme_note_finale    
...
def list_students_admis(anneeacademique_id, salle_id, trimestre):
    ...
    for student in students: 
        ...
        if moyenne_generale >= deliberation.moyennevalidation:
            ...
            dic["details"] = detail_note_student(anneeacademique_id, salle_id, trimestre, student.id)
            dic["somme_totale_notes"] = cacluler_somme_finale_notes(anneeacademique_id, salle_id, trimestre, student.id)
...
def comparaison_note_matiere(anneeacademique_id, salle_id, student_id):
    ...
    for tm in trimestres_compositions: 
            ...
            dic["moyenne"] = moyenne_generale
            dic["details"] = detail_note_student(anneeacademique_id, salle_id, tm["trimestre"], student_id)
...
def gestion_etude(request):
    ...
    for tc in trimestres_compositions:
        ...
        dic["moyenne"] = moyenne_generale
        dic["mention"] = mention_student(moyenne_generale)
        dic["details"] = detail_note_student(anneeacademique_id, inscription.salle.id, tc["trimestre"], student_id)
        dic["somme_totale_notes"] = cacluler_somme_finale_notes(anneeacademique_id, inscription.salle.id, tc["trimestre"], student_id)
        # Somme des coefficients
...
def gestion_etude_parent_detail(request, student_id):
    ...
    # Absence des étudiants
    absences = Absencestudent.objects.filter(student_id=student_id, status_parent__in=[0,1])
    for absence in absences:
        if absence.emargement.anneeacademique.id == anneeacademique_id:
    ...
    # Absence des étudiants
    new_absences = Absencestudent.objects.filter(student_id=student_id)
    tababsences = []
    for absence in new_absences:
        if absence.emargement.anneeacademique.id == anneeacademique_id:      
            tababsences.append(absence)
    ...
    # Mettre à jour le status de composition de l'étudiant pour marque la lecture
    compositions_students = Composer.objects.filter(student_id=student_id, anneeacademique_id=anneeacademique_id, status_parent__in=[0,1])
    for composition in compositions_students:
    ...
    return render(request, "gestion_etude_parent_detail.html", context=context)
...
def detail_cmpteacher(request, salle_id, matiere_id, trimestre):
    ...
    return render(request, "detail_cmpteacher.html", context=context)
...
def bulletin_etudiant(request, student_id, trimestre):
    ...
    # Resultats
    moyenne_generale = avg_student(anneeacademique_id, inscription.salle.id, trimestre, inscription.student.id)
    mention = mention_student(moyenne_generale)
    details = detail_note_student(anneeacademique_id, inscription.salle.id, trimestre, inscription.student.id)
    somme_totale_notes = cacluler_somme_finale_notes(anneeacademique_id, inscription.salle.id, trimestre, inscription.student.id)
    # Somme des coefficients
    somme_coefficient = calculer_somme_coefficient(anneeacademique_id, inscription.salle.id, trimestre)
    # Nombre des étudiants qui ont composé
    nb_students_inscris = nombre_student_inscris(anneeacademique_id, inscription.salle.id)   
    # Nombre d'étudiants qui ont composé
    nb_student_compose = nombre_student_composer(anneeacademique_id, inscription.salle.id, trimestre)   
    # Récuperer les élèves inscris dans cette salle
    students = students_inscris(inscription.salle.id, anneeacademique_id)
    ...
    moyenne_students = []
    for student in students:  
            my = avg_student(anneeacademique_id, inscription.salle.id, trimestre, student.id)          
            moyennes.append(my)   
            dic_rang = {}
            dic_rang["student_id"] = student.id
            dic_rang["moyenne"] = my
    ...
    # Récuperer la délibération
    deliberation = Deliberation.objects.filter(anneeacademique_id=anneeacademique_id, salle_id=inscription.salle.id, trimestre=trimestre).first() 
    # Nombre d'absences
    nb_absences = 0
    absences = Absencestudent.objects.filter(student_id=student_id)
    for absence in absences:
    ...
    context = {
        ...
        "details": details,   
        "somme_totale_notes": somme_totale_notes,    
        "somme_coefficient": somme_coefficient,
...
def proces_verbal_examen(request, salle_id, trimestre):
        ...
        nb_students_inscris = nombre_student_inscris(anneeacademique_id, salle_id)
        # Nombre des étudiants qui ont composé
        nb_student_compose = nombre_student_composer(anneeacademique_id, salle_id, trimestre)
        # Moyenne minimal et maximal
        moyenne_min = min(moyennes)
        moyenne_max = max(moyennes) 
        # Récuperer la salle
        ...
        pdf = pdfkit.from_string(html, False, options)
        reponse = HttpResponse(pdf, content_type='application/pdf')
        reponse['Content-Disposition'] = f"attachment; filename=Proces_{ salle }_{ trimestre }.pdf"
        return reponse    
...
def releve_note_controle(request, salle_id, num_controle, matiere_id, trimestre):
        ...
        pdf = pdfkit.from_string(html, False, options)
        reponse = HttpResponse(pdf, content_type='application/pdf')
        reponse['Content-Disposition'] = f"attachment; filename=Releve_note_{ salle }_{ trimestre }.pdf"
        return reponse
...
def releve_note_examen(request, salle_id, matiere_id, trimestre):
        ...
        pdf = pdfkit.from_string(html, False, options)
        reponse = HttpResponse(pdf, content_type='application/pdf')
        reponse['Content-Disposition'] = f"attachment; filename=Proces_{ salle_id }_{ trimestre }.pdf"
        return reponse 
        ...
        if deliberation is None:
            ...
            if user.check_password(password):
                ...
                return JsonResponse({
                            ...
                            "message": "Délibération cloturée avec succès."})
            else:
                return JsonResponse({
                            "status": "error",
                            "message": "Mot de passe incorrect."})