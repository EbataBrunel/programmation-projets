...
# Montant du contrat sur le mois
def montant_contrat_month(month, user_id, type_contrat, anneeacademique_id):
    montant = 0
    ...
    for contrat in contrats:
        # Recuperer la liste des mois du contrat
        months = month_contrat_user(contrat)
        if month in months:
            montant = contrat.amount
            break            
    return montant

# Poste du contrat sur le mois
def poste_contrat_month(month, user_id, type_contrat, anneeacademique_id):
    poste = ""
    contrats = Contrat.objects.filter(
        user_id=user_id, 
        type_contrat=type_contrat, 
