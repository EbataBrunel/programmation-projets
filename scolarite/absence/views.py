...
def presence_student(request, id):
    ...
    for inscription in inscriptions:
        ...
        query = Absencestudent.objects.filter(emargement_id=id, student_id=inscription.student.id)
        if query.exists():
            dic["presence"] = "Absent"
        else:
...
def abs_student_mat_user(request, student_id):
    ...
    for ed in emargements_dates:    
        ...
        absences = []
        for emargement in emargements:
            query = Absencestudent.objects.filter(emargement_id=emargement.id, student_id=student_id)
            if query.exists():
                absence = Absencestudent.objects.filter(emargement_id=emargement.id, student_id=student_id).first()
