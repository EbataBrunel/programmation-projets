                ...
                for cours in list_cours:
                    ...
                    for comment in comments:
                        ...
                        if commentaire.author_content_type != student_type:
                            ...
                            if dic_comment:
                                if user_id not in dic_comment.values():
                                    nb_newcomment += 1
                            else:
                                nb_newcomment += 1
...
def detail_cours(request, salle_id, matiere_id):
    ...
    salle = Salle.objects.get(id=salle_id)
    matiere = Matiere.objects.get(id=matiere_id)
    list_cours = Cours.objects.filter(salle_id=salle_id, matiere_id=matiere_id, enseignant_id=user_id, anneeacademique_id=anneeacademique_id)
    """
    if cours.count() > 0:
            dic = {}
            dic["matiere"] = matiere
...
def edit_cours(request, id):
    ...
    query = Cours.objects.filter(id=id, enseignant_id=request.user.id)
    if query.exists():
            ...
            for cours in list_cours:
                ...
                for comment in comments:
                    ...
                    if commentaire.author_content_type != student_type:
                        dic_comment = eval(comment.reading_status)
                        if dic_comment:
                            if student_id not in dic_comment.values():
                                nb_newcomment += 1
                        else:
                            nb_newcomment += 1
...
def detail_coursligne(request, salle_id, matiere_id):
    ...
    for c in cours:
        ...
        for comment in comments:
            ...
            if commentaire.author_content_type != student_type:
                dic_comment = eval(comment.reading_status)
                if dic_comment:
                    if request.user.id not in dic_comment.values():
                        nb_newcomment += 1
                else:
                    nb_newcomment += 1
    ...
    liste_cours = Cours.objects.filter(salle_id=salle_id, matiere_id=matiere_id, anneeacademique_id=anneeacademique_id)
    for cours in liste_cours:
        comments = CommentCours.objects.filter(cours=cours)
        for com in comments:
            comment = get_object_or_404(CommentCours, id=com.id)
            # Vérifier si l'auteur est un étudiant
            student_type = ContentType.objects.get_for_model(Student)  # Type de modèle pour un étudiant
            if comment.author_content_type != student_type:
                dic_comment = eval(comment.reading_status)
                d = {}
                if dic_comment:
                    for key in dic_comment.values():
