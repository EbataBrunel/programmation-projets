...
def contact_sp_admin(request, customer):
    ...
    if customer not in ["student", "parent"]:
...
def contact_admin_detail(request, customer, id):
    ...
    if customer == "student":
        ...
        # Mise à jour de reading_status pour marquer la lecture des messages
        contacts = Contact.objects.filter(anneeacademique_id=anneeacademique_id, student_id=id, user_id=user_id, sending_status=False, reading_status__in=[0,1])
        for cont in contacts:
            if cont.student.id: # Verifier s'il s'agit bien du contact de l'étudiant
                cont.reading_status = 2
                cont.save()
                
        contacts = Contact.objects.filter(anneeacademique_id=anneeacademique_id, student_id=id, user_id=user_id).order_by("id")
        # Liste des sujets
        subjects = ['Réclamation de notes', 'Harcèlement', 'Paiement des frais'] 
        context = {
            "customer": customer,
...
def messages(request):
    ...
    # Tri de la liste des datemessage par ordre décroissante
...
def detail_message(request, id):
    ...
    messages = Message.objects.filter(
            Q(expediteur=user_id, beneficiaire=id) |
            Q(expediteur=id, beneficiaire=user_id)
        ).order_by('datemessage')
