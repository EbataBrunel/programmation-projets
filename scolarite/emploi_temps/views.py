...
def add_emploitemps(request):
    ...
    for user in users:
        ...
        for group in groups:
            if group.name in permission_enseignant:
...
def edit_emploitemps(request, id):
    ...
    matieres = Matiere.objects.exclude(id=emploitemps.matiere.id)
    enseignanats = User.objects.filter(is_superuser = False).exclude(id=emploitemps.enseignant.id)
    jours = ["Lundi", "Mardi", "Mercredi", "Jeudi", "Vendredi", "Samedi", "Dimanche"]
    tabJours = []
    for jour in jours:
...
class get_matiere_programmer_salle_emploi(View):
