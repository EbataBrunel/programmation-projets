...
def detail_inscription(request, id):
    ...
    somme_inscription = (Inscription.objects.filter(salle_id=id, anneeacademique_id=anneeacademique_id).aggregate(Sum("amount"))["amount__sum"] or 0)
    anneeacademique = AnneeCademique.objects.get(id=anneeacademique_id)
    context = {
        "setting": setting,
...
def add_inscription(request):
    ...
    if request.method == "POST":   
        ...
        query = Inscription.objects.filter(student_id=student_id, anneeacademique_id=anneeacademique_id)    
        # Récuperer la salle 
        ...
        if salle.price != amount:
             return JsonResponse({
                "status": "error",
                "message": "Le montant renseigné ne correspond pas à celui de la salle."})   
...
def edit_inscription(request,id):
    ...
    students = Student.objects.exclude(id=inscription.student.id)
    salles = Salle.objects.exclude(id=inscription.salle.id)
    mode_paiements = ["Espèce", "Virement", "Mobile"]
    tab_mode_paiements = []
...
def edit_in(request):
    if request.method == "POST":
        ...
        if inscription is None:
            ...
            anneeacademique_id = request.session['anneeacademique_id'] 
            salle_id = request.POST["salle"]
            student_id = request.POST["student"]
            amount = bleach.clean(request.POST["amount"].strip())
            ...
            if anneescolaire.exists(): # Verifier si on a déjà cloturé les opérations de cette année
                return JsonResponse({
                    "status": "error",
                    "message": "Les opérations de cette année académique ont déjà été clôturées."})        
            if salle.price != amount: # Verifier si le montant corresponds à celui de la salle
                return JsonResponse({
                    "status": "error",
                    "message": "Le montant renseigné ne correspond pas à celui de la salle."})
...
def attestation_inscription(request, student_id):
    ...
    for user in users:
        if user.groups.exists():
            ...
            for group in groups:
                    if group.name in ["Directeur des Etudes"]:                       
                            user = user
